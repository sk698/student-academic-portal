  ...
  return (
    <>
      <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <button className="rounded-xl border border-outline-variant/40 bg-white px-5 py-3 text-sm font-semibold text-primary">Mark all as read</button>
      </header>
      ...
      <section className="grid grid-cols-1 gap-6 md:grid-cols-12">
        <article className="relative overflow-hidden rounded-2xl bg-surface-container-lowest p-7 shadow-[0_12px_32px_-8px_rgba(0,63,135,0.08)] md:col-span-8">
          ...
          <div className="mb-5 flex items-center justify-between gap-2">
            ...
            <time className="text-xs font-medium text-outline">{announcementHero.date}</time>
          </div>
          <h2 className="font-headline text-2xl font-bold tracking-tight text-primary">{announcementHero.title}</h2>
          <p className="mt-4 text-sm leading-relaxed text-on-surface-variant">{announcementHero.content}</p>
        ...
        <article className="relative overflow-hidden rounded-2xl bg-primary p-7 text-white shadow-[0_12px_32px_-8px_rgba(0,63,135,0.22)] md:col-span-4">
          ...
          <h3 className="mt-4 font-headline text-2xl font-bold leading-tight">{announcementFeatured.title}</h3>
          <p className="mt-3 text-sm text-blue-100">{announcementFeatured.content}</p>
        ...
        <div className="grid grid-cols-1 gap-5 md:col-span-12 md:grid-cols-3">
          {announcements.map((item) => (
            <article className="rounded-2xl bg-surface-container-lowest p-6 shadow-[0_10px_28px_-10px_rgba(0,63,135,0.12)]" key={item.id}>
